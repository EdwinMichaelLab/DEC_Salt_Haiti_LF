function [MfPrevStore,CfaPrevStore,MBRArray,L3Array] = ...
    MDA_Intervention_w_VCLLIN(beta,alpha,k0,kLin,k1,r1,sigma1,psi1,...
    psi2s2,mu,gamma,b1,c,ageLev,k2,gam2,immC,slopeC,VoverH,del,...
    PP,alpha2,gamma2,ageMthMax,da,bCulex,l3,gVec,pVec,...
    wVec,mVec,iVec,cVec,demog,AgeLimits0,MonthlyMDACov,...
    RegimenEfficacy,MDAInterval,NumYears,VCparams,...
    IRSCoverages,ITNCoverages,SwitchMonth)

SexRatio = 0.5;

IRScoverage = VCparams(5);
IRS_RepelEff = VCparams(1); IRS_KillEff = VCparams(2);
IRS_EfficacyDecayRate = VCparams(3); IRSreplacementPeriod = VCparams(4);

ITNcoverage = VCparams(11); ITN_DeterEff = VCparams(6);
ITN_RepelEff = VCparams(7); ITN_KillEff = VCparams(8);
ITN_EfficacyDecayRate = VCparams(9); ITNreplacementPeriod = VCparams(10);
...
CfaPrevStore = zeros(1,1+12*NumYears,'single');
MBRArray = zeros(1,1+12*NumYears,'single');
L3Array = zeros(1,1+12*NumYears,'single');
...
sumC = sumMf(c2,ageMthMax,da,demog);
CfaPrevStore(1) = 100*mfAgeprevFun(sumC,negBinshapeFun(sumC,k0,kLin));
...
for iTime = 1:da:NumYears*12
    ...
    for a2 = 2:da:PP
        ...
        cVec(a2) = c1 + (alpha2*w1-gamma2*c1)*da;
    ...
    if (mod(iTime,MDAInterval(iTime)) == 1) || MDAInterval(iTime) == 1
        sinceTreat = 1;
        for a2 = PP+1:da:ageMthMax
            ...
            if (a2 >= AgeLimits(1)*12 && a2 < AgeLimits(2)*12)
                gVec(a2) = (1-drug(1)*MonthlyMDACov(iTime))*gVec(a2); % why do we use drug effect here??
    ...
    sumC = sumMf(cVec,ageMthMax,da,demog);
    CfaPrevStore(1,iTime+1) = 100*mfAgeprevFun(sumC,negBinshapeFun(sumC,k0,kLin));
