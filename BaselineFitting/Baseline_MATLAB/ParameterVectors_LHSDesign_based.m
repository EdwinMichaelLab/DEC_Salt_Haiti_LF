...
for i=1:length(minParamVals)
    ParameterVectors(i,:) = minParamVals(i) + ...
        rangeParamVals(i)*lhsdesign(1,NParamVecs,'criterion','correlation');  % technically this is not uniform sampling, instead it's taking fixed interval between min & max
end

ParameterVectors(14,:) = floor(ParameterVectors(14,:)+0.5); % ageLev should be non-fraction
ParameterVectors(20,:) = floor(ParameterVectors(20,:)+0.5); % PP should be non-fraction, Prepatency Period
